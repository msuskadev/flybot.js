import { AttachmentLayoutTypes, CardFactory, MessageFactory, StatePropertyAccessor, TurnContext, ConversationState } from 'botbuilder';
import {
    ComponentDialog,    
    DialogSet,
    DialogState,
    DialogTurnResult,
    DialogTurnStatus,
    TextPrompt,
    WaterfallDialog,
    WaterfallStepContext,
} from 'botbuilder-dialogs';
import FlyBotService from "../services/flybot.service";
import FlightModel from '../models/flight.model';
import DialogPropertiesModel from '../models/dialog-properties.model';
import moment from 'moment';
import CardFormatter from '../utils/card-formatter';
import TripHelper from '../utils/trip-helper';
import SearchFlightCard from '../assets/search-flights-card.json';

const SEARCH_FLIGHTS_DIALOG_CARD = 'SEARCH_FLIGHTS_DIALOG_CARD';
const WATERFALL_SEARCH_FLIGHTS_DIALOG_CARD = 'WATERFALL_SEARCH_FLIGHTS_DIALOG_CARD';
const SHOW_CARD = 'SHOW_CARD';

export class SearchDomesticFlightsDialog extends ComponentDialog {
    private dialogProperties!: DialogPropertiesModel;

    constructor() {
        super(SEARCH_FLIGHTS_DIALOG_CARD);

        this.addDialog(new TextPrompt(SHOW_CARD))            
            .addDialog(new WaterfallDialog(WATERFALL_SEARCH_FLIGHTS_DIALOG_CARD, [
                this.showCard.bind(this),                
                this.finalStep.bind(this),
            ]));
        
        this.initialDialogId = WATERFALL_SEARCH_FLIGHTS_DIALOG_CARD;
    }

    public async run(context: TurnContext, accessor: StatePropertyAccessor<DialogState>, dialogProperties: StatePropertyAccessor<DialogPropertiesModel>) {
        const dialogSet = new DialogSet(accessor);
        dialogSet.add(this);

        this.dialogProperties = await dialogProperties.get(context, { active: false });
        const dialogContext = await dialogSet.createContext(context);
        const results = await dialogContext.continueDialog();
        if (results.status === DialogTurnStatus.empty) {
            this.dialogProperties.active = true; 
            await dialogContext.beginDialog(this.id);
        }
    }

    private async showCard(step: WaterfallStepContext) : Promise<DialogTurnResult> {    
        return await step.prompt(SHOW_CARD, {
            prompt: MessageFactory.attachment(CardFactory.adaptiveCard(await this.prepareCard())),
            retryPrompt: 'Something went wrong'
        });
    }

    private async finalStep(step: WaterfallStepContext) : Promise<DialogTurnResult>{        
        const params = step.context.activity.value;

        if (params) {
            await step.context.sendActivities([
                { type: 'typing' }]);

            const date = moment(params.TravelDate).format("DD/MM/YYYY");
            const fm: FlightModel = {} as any;
            fm.flyFrom = params.DepartureAirport;
            fm.flyTo = params.ArrivalAirport;
            fm.dateFrom = date;
            fm.dateTo = date;
            if (params.TravelAloneOrWithFamily === 'TravelAlone') {
                fm.adults = 1;
                fm.children = 0;
                fm.infants = 0;    
            } else {
                fm.adults = params.AdultsNumber;
                fm.children = params.ChildrenNumber;
                fm.infants = 0;
            }         

            const allFlights = await new FlyBotService().searchFlights(fm as FlightModel);

            if (allFlights && allFlights.length > 0) {            
                await step.context.sendActivity({
                    attachments: [
                    CardFactory.heroCard(
                    'Fastest Flights',
                    CardFactory.images(['https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQA-U0NX5KEftXustpxV0Po39vlCC5ytRmdqQ&usqp=CAU'])
                )]});

                await step.context.sendActivity({
                    attachmentLayout: AttachmentLayoutTypes.Carousel,
                    attachments: [                        
                        ...CardFormatter.format(TripHelper.getFastestFlights(allFlights))
                    ]
                });

                await step.context.sendActivity({
                    attachments: [
                    CardFactory.heroCard(
                    'Cheapest Flights',
                    CardFactory.images(['data:image/jpeg;base64,'])
                )]});

                await step.context.sendActivity({
                    attachmentLayout: AttachmentLayoutTypes.Carousel,
                    attachments: [
                        ...CardFormatter.format(TripHelper.getCheapestFlights(allFlights))
                    ]
                });

                await step.context.sendActivities([
                    MessageFactory.text('Enjoy your trip! Safe flight!')
                ]);
            } else {
                await step.context.sendActivities([
                    MessageFactory.text(`There's no available flights!`)
                ]);
            }     
        }
        
        this.dialogProperties.active = false;
        return await step.endDialog();
    }    

    private async prepareCard() {
        const allAirports = (await (new FlyBotService()).getAirports('PL'));

        const cardBody = SearchFlightCard;
        let departureAirport = cardBody.body.find(el => el.id === 'DepartureAirport');
        let arrivalAirport = cardBody.body.find(el => el.id === 'ArrivalAirport');
        let adultsNumber = cardBody.body.find(el => el.id === 'AdultsNumber');
        let childrenNumber = cardBody.body.find(el => el.id === 'ChildrenNumber');
        
        if (departureAirport) {
            departureAirport.choices = allAirports;
        }
    
        if (arrivalAirport) {
            arrivalAirport.choices = allAirports;
        }

        if (adultsNumber) {
            adultsNumber.choices = this.prepareInput(0, 6);
        }

        if (childrenNumber) {
            childrenNumber.choices = this.prepareInput(0, 10);
        }
        return cardBody;
    }

    private prepareInput(start: number, end: number) {
        const result = [];

        for (let i = start; i <= end; i++) {
            result.push({ title: `${i}`, value: `${i}` });
        }

        return result;
    }
}